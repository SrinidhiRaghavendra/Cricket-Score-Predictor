...
def insert(inn,bat, ground, ball, score, data) :
    if bat not in data.keys() :
        data[inn][bat] = {}
    if ground not in data[inn][bat].keys() :
        data[inn][bat][ground] = []
        for i in range(121) :
            tmp = {}
            for i in range(-1, 8) :
            ...
            data[inn][bat][ground].append(tmp)
    try:        
        data[inn][bat][ground][ball][str(score)] += 1
    except:
        pass
...
def save_inning(inn_no, _inn, ground, data) :
    ...
    for i in inn :
        ...
        insert(inn_no, j["batsman"], ground, ball, score, data)
        ball += 1
...
f = open("../../data/json/bat_ground_1.json","w")
f.write(json_data_1)
f.close()
f = open("../../data/json/bat_ground_2.json","w")
f.write(json_data_2)
f.close()
